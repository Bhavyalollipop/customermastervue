<template>
  <div class="mt-3">
    ...
    <b-form v-if="show" class="mt-3" @submit.prevent="onSubmit">
      ...
      <b-table-simple v-if="form.cus_addr.length > 0" class="mt-2" hover small responsive>
        ...
        <b-tbody>
          <b-tr v-for="(item, index) in form.cus_addr" :key="index">
            <b-td><span class="cursor-pointer" @click="updateAddressmodal(item)"> <u> {{ item.cus_addr_line_1 }} </u></span> </b-td>
      ...
      <addressModal :modal-show="modalShow" :update="update" :addressedit="addressedit" @add="addAdress" @close="modalShow = false" />
...
export default {
  ...
  methods: {
    async updateAddressmodal (item) {
      this.update = await true
      this.addressedit = await item
      this.modalShow = await true
    },
    getCustomerList () {
      this.$store.dispatch('customer/view', { id: this.$route.params.id }).then((res) => {
        const form = res.cus_main
        form.cus_addr = res.cus_addr
        this.form = form
      })
    },
    async addAdress (data) {
      if (this.update) {
        const mapFn = (item) => {
          if (item.cus_addr_id === data.cus_addr_id) {
            item = data
          }
          return item
        }
        this.form.cus_addr = await this.form.cus_addr.map(mapFn)
      } else {
        await this.form.cus_addr.push(data)
      }
      this.update = await false
      this.modalShow = await false
    },
    onSubmit () {
      this.$store.dispatch('customer/update', { id: this.$route.params.id, form: this.form }).then((res) => {
        if (res.type === 'success') {
